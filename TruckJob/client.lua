...
function ThirdTask()
    local payAmount = math.random(Config.MinPayAmount, Config.MaxPayAmount)
    ...
    while true do
        ...
        if #(GetEntityCoords(player) - vector3(Config.DepotLocation.x, Config.DepotLocation.y, Config.DepotLocation.z)) <= 10 then
            ...
            if IsControlPressed(1, 38) then
                ...
                DeleteVehicle(trailer)
                isOnJob = false
                if not hasCanceledJob then
                    if Config.UseND then
                        TriggerServerEvent("LamasJobs:GivePay", payAmount)
                        DisplayNotification("You've received ~g~$" .. payAmount .. " ~w~for completing the job.")
                        break
                    else
