...
AddEventHandler("LamasJobs:GivePay", function(payAmount)
    local player = source
    NDCore.Functions.AddMoney(payAmount, player, "bank")
end)

RegisterServerEvent("LamasJobs:GivePenalty")
AddEventHandler("LamasJobs:GivePenalty", function(penaltyAmount)
    local player = source
    NDCore.Functions.DeductMoney(penaltyAmount, player, "bank")
end)
